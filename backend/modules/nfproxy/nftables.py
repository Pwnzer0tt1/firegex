...
class FiregexTables(NFTableManager):
    ...
    def add(self, srv:Service, queue_range):
        ...
        if srv.tls_enabled:
            ...
            self.cmd(
                ...
                {"insert":{"rule":{ # Send cleartext inbound traffic to NFQUEUE
                    ...
                    "expr": [
                            ...
                            {'match': {'left': {'payload': {'protocol': convert_protocol_to_l4(str(srv.proto)), 'field': 'dport'}}, 'op': '==', 'right': int(clear_port)}},
                ...
                {"insert":{"rule":{ # Send cleartext outbound traffic to NFQUEUE
                    ...
                    "expr": [
                            ...
                            {'match': {'left': {'payload': {'protocol': convert_protocol_to_l4(str(srv.proto)), 'field': 'sport'}}, 'op': '==', 'right': int(clear_port)}},
            ...
            self.cmd(
                { "insert":{ "rule": { # Send non-TLS outbound traffic to NFQUEUE
                    "family": "inet",
                    "table": self.table_name,
                    "chain": self.output_chain,
                    "expr": [
                ...
                {"insert":{"rule":{ # Send non-TLS inbound traffic to NFQUEUE
                    "family": "inet",
                    "table": self.table_name,
                    "chain": self.input_chain,
                    "expr": [
