...
def log_print(module:str, *args, level:str = LogLevels.INFO, **kwargs):
    return print(f"{load_level_str(level)}[deep_pink4 bold]\\[pyfilters][/][medium_orchid3 bold]\\[{escape(module)}][/]", *args, **kwargs)
        ...
        while True:
            ...
            if has_to_filter:
                filter_ctx["__firegex_packet_info"] = {
                    ...
                    "is_ipv6": is_ipv6,
                    "is_tcp": is_tcp,
                    # Real addresses, because this proxy knows them. There is no
                    # invented header any more: a filter reads metadata and edits the
                    # payload, which is the same contract it gets in production.
                    "src_ip": endpoints["client_ip"] if is_input else endpoints["server_ip"],
                    "src_port": endpoints["client_port"] if is_input else endpoints["server_port"],
                    "dst_ip": endpoints["server_ip"] if is_input else endpoints["client_ip"],
                    "dst_port": endpoints["server_port"] if is_input else endpoints["client_port"],
                }
                try:
                    exec("firegex.pyfilters.internals.handle_packet(globals())", filter_ctx, filter_ctx)
...
async def _handle_connection(
    ...
    endpoints = {
        ...
        "server_ip": target_ip, "server_port": target_port,
    }
    try:
    ...
    task1 = asyncio.create_task(_forward_and_filter(filter_ctx, reader, remote_writer, True, ipv6, True, endpoints, True))
    task2 = asyncio.create_task(_forward_and_filter(filter_ctx, remote_reader, writer, False, ipv6, True, endpoints, True))
    try:
...
def _build_filter(filepath:str):
    ...
    # No protocol is named here, or anywhere else: the file shows which one it speaks by
    # what its filters ask for. Naming it in a second place is how a file and its
    # declaration drift apart, and the drift shows up as a filter that never runs.
    filters = get_filter_names(filter_code)
    filter_code += (
...
def run_proxy_simulation(filter_file:str, target_ip:str, target_port:int, local_ip:str = None, local_port:int = 7474, ipv6:bool = False):
    ...
    if os.path.isfile(filter_file) is False:
        raise Exception(f"\\[pyfilters]\\[init] Filter file {filter_file} not found")
    else:
    ...
    def reload_proxy_proc():
        ...
        try:
            compiled_filter = _build_filter(filter_file)
        except Exception:
