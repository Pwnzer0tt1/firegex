...
def composecmd(cmd):
    return os.system(f"(hash docker-compose &> /dev/null && docker-compose -p firegex {cmd}) || (hash docker &> /dev/null && docker compose -p firegex {cmd}) || echo 'Docker not found!, please install docker and docker-compose'")

def dockercmd(cmd):
    return os.system(f"(hash docker &> /dev/null && docker {cmd}) || echo 'Docker not found!, please install docker and docker-compose'")
