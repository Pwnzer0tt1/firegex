import { Button, Group, Space, TextInput, Notification, Modal, Switch, SegmentedControl, Box, Tooltip, ActionIcon, Title, Tabs, Text, UnstyledButton, Card, Divider } from '@mantine/core';
...
function AddEditService({ opened, onClose, edit }:{ opened:boolean, onClose:()=>void, edit?:Service }) {
    ...
    const submitRequest = async (values: ServiceAddForm) => {
        ...
        if (edit){
            ...
            nfproxy.settings(edit.service_id, { port: values.port, proto: values.proto, ip_int: values.ip_int, fail_open: values.fail_open, target_type: values.tls_enabled ? "tls" : "flow", tls_stream_id: streamId }).then( res => {
                if (!res){
                    setSubmitLoading(false)
                    close();
            ...
            <Tabs defaultValue="general" color="cyan" variant="outline" radius="md">
                ...
                <Tabs.Panel value="general" pt="xl" pb="md">
                    ...
                    <Group grow mb="xl">
                        ...
                        <UnstyledButton
                            onClick={() => form.setFieldValue('tls_enabled', true)}
                            style={{
                                padding: '1rem',
                                borderRadius: '12px',
                                border: !form.values.tls_enabled ? '1px solid var(--fourth_color)' : '2px solid var(--accent-color)',
