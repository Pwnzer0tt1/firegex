"""The filter library's own behaviour, with no instance and no libhs.

The HTTP models, the knobs a filter file is documented to be able to set, and the order
its functions are collected in — all of which the datapath depends on and none of which
needs anything running to check.
"""

from firegex.pyfilters import pyfilter, ACCEPT, ExceptionAction, collect_pyfilters
from firegex.pyfilters.models import HttpHistory, HttpStreamHistory
...
"""
    ...
    assert recorded[1] == ("resp", 200, ["/first"], [])

    # 3. Second Request: past requests contains [/first], past responses contains [200]
    assert recorded[2] == ("req", "/second", ["/first"], [200])

    # 4. Second Response: past requests contains [/first, /second], past responses contains [200]
    assert recorded[3] == ("resp", 404, ["/first", "/second"], [200])
