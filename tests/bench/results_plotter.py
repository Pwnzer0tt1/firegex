"""Redraws the charts in `results/` from the CSVs beside them.

The archived half reads data recorded by `comparemark.py`, which is gone — it drove the
`nfregex`/`nfproxy` API from before both were folded into `services`. The CSVs it left
are a record of what was measured then, so they are still plottable; there is just no
longer a way to produce new ones, which is why that half is behind `--archived`.

Paths are resolved against this file rather than the working directory, so it draws the
